...
| ------------- | ----------- | --- | --- | ----- |
| [fivetran_platform__sync_metrics](https://fivetran.github.io/dbt_fivetran_log/#!/model/model.fivetran_log.fivetran_platform__sync_metrics) | New model |  |  | Returns one record per completed sync, combining each sync's final status (`sync_status`) with its extract, process, and load timing and volume statistics, total duration (`total_time_s`), rows extracted from the source (`total_extracted_rows`), and total records modified (`total_loaded_rows`), enriched with connection and destination details. Connections that do not emit a `sync_stats` log event still appear, but with null timing and volume metrics. |
...
| [fivetran_platform__audit_table](https://fivetran.github.io/dbt_fivetran_log/#!/model/model.fivetran_log.fivetran_platform__audit_table) | New field |  | `rows_loaded` | Adds the total number of rows written to the table during the sync, calculated as `sum_rows_replaced_or_inserted` + `sum_rows_updated` + `sum_rows_deleted`. |
...
## Bug Fix
- Resolves duplicate records in `fivetran_platform__audit_table` arising out of multiple `records_modified` events for a single table during a sync. ([PR #174](https://github.com/fivetran/dbt_fivetran_log/pull/174))
