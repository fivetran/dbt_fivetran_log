...
- Adds the optional `fivetran_platform_lookback_window_months` variable to control how much source log data is scanned on each run. When set to an integer, `stg_fivetran_platform__log` filters to the last N months from the current run time — recalculated on every run, not just the initial load. Unset by default — all log history is included unless configured. See the [README](https://github.com/fivetran/dbt_fivetran_log/blob/main/README.md#limit-the-lookback-window) for details. Affects `fivetran_platform__connection_status`, `fivetran_platform__schema_changelog`, `fivetran_platform__audit_user_activity`, `fivetran_platform__connection_daily_events`, and `fivetran_platform__audit_table` (**note:** incremental downstream models accumulate history over time on regular runs, but a `--full-refresh` is bounded by N months of history; run `dbt run --full-refresh` after setting or changing this variable).
