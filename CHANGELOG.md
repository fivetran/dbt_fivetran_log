...
| [fivetran_platform__connection_event_variance](https://fivetran.github.io/dbt_fivetran_log/#!/model/model.fivetran_log.fivetran_platform__connection_event_variance) | New model |  |  | Returns daily event volume per connection and event subtype, comparing each day against the average volume for that connection, event subtype, and weekday and flagging days that fall outside a configurable variance band. The high and low sides of the band are set by the new `fivetran_platform_event_high_variance_percent` and `fivetran_platform_event_low_variance_percent` variables (both default to `0.2`, or ±20%). |
| [fivetran_platform__records_modified_variance](https://fivetran.github.io/dbt_fivetran_log/#!/model/model.fivetran_log.fivetran_platform__records_modified_variance) | New model |  |  | Returns each record modification (`records_modified`) event per connection and table (excluding the internal `fivetran_audit` table), comparing the rows modified against the average for that connection, table, operation type, and weekday and flagging events that fall outside a configurable variance band. The high and low sides of the band are set by the new `fivetran_platform_records_modified_high_variance_percent` and `fivetran_platform_records_modified_low_variance_percent` variables (both default to `0.2`, or ±20%). |
