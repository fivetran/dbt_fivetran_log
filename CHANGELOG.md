...
| ------------- | ----------- | --- | --- | ----- |
| [fivetran_platform__sync_metrics](https://fivetran.github.io/dbt_fivetran_log/#!/model/model.fivetran_log.fivetran_platform__sync_metrics) | New model |  |  | Returns one record per completed sync (`sync_stats` log event), combining each sync's extract, process, and load timing and volume statistics with its total duration (`total_time_s`) and total records modified (`rows_modified_count`), enriched with connection and destination details. |
