...
| [fivetran_platform__sync_metrics](https://fivetran.github.io/dbt_fivetran_log/#!/model/model.fivetran_log.fivetran_platform__sync_metrics) | Captures one record per completed sync, combining each sync's extract, process, and load timing and volume statistics with its total duration and total records modified, enriched with connection and destination details to analyze sync performance and throughput over time. |
