...
| [fivetran_platform__sync_metrics](https://fivetran.github.io/dbt_fivetran_log/#!/model/model.fivetran_log.fivetran_platform__sync_metrics) | Captures one record per completed sync, combining each sync's final status with its extract, process, load timing and volume statistics, total duration, and total rows extracted and loaded, enriched with connection and destination details to analyze sync performance and throughput over time. Not every connection emits a `sync_stats` log event. Syncs without one still appear in this model, but their timing and volume metrics are null. |
