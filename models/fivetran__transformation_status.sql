{{ config(enabled=var('fivetran_using_transformations', True)) }}

with transformation as (

    select *
    from {{ ref('stg_fivetran__transformation') }}
),
...
{% if var('fivetran_using_triggers', True) %}
trigger_tables as (
    ...
    from {{ ref('stg_fivetran__trigger_table') }}
