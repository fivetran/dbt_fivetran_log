...
join_usage_mar as (
    ...
    select 
        ...
        round( cast(nullif(usage.consumption,0) * 1000000.0 as {{ dbt_utils.type_float() }}) / cast(nullif(destination_mar.monthly_active_rows,0) as {{ dbt_utils.type_float() }}), 2) as usage_per_million_mar,
        round( cast(nullif(destination_mar.monthly_active_rows,0) * 1.0 as {{ dbt_utils.type_float() }}) / cast(nullif(usage.consumption,0) as {{ dbt_utils.type_float() }}), 0) as mar_per_usage
