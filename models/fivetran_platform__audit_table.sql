{{ config(
    ...
    incremental_strategy='insert_overwrite' if target.type in ('bigquery') or not is_databricks_sql_warehouse(target) else ('merge' if is_databricks_sql_warehouse(target) else 'delete+insert'),
